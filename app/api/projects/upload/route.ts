    ...
    try {
      ...
      const created = await prisma.mPProject.createMany({
        data: transformedData.map((proj: any) => ({
          ...
          startDate: (() => { const d = proj.startDate ? new Date(proj.startDate) : null; return d && !isNaN(d.getTime()) ? d : new Date() })(),
          endDate: (() => { const d = proj.endDate ? new Date(proj.endDate) : null; return d && !isNaN(d.getTime()) ? d : new Date() })(),
